...
//-------------------------------use of UDCORE: first layer--------------------------------------
/*concentrates most of the functionality and complexity. Its functions range from
 drawing lines and shapes, to the management and interaction with the elements of the diagram,
 all regardless of the characteristics inherent in UML*/

//----------------------------------------Elements---------------------------------------------------
//--------Diagram	: drawing canvas and internal logic+organize the plot elements and manipulate them+management of events(user)
//--------Node	 	: move, resize and modifying(elements in the diagram and relationships that are not inherited from this class)
//					  Elements:class/package
//-------- SuperNode: composed of two or more objects Node.
//--------Relation	: relationships between the different elements of the diagram, whether nodes or relationships.
//--------NodeFigure: graphical representation of a node
...
Node.prototype.showStyleDialog = function(e) {
    ...
    y.add(new Option("Bolder", "bolder")); /*http://www.jrromero.net/tools/jsUML2*/
...
Relation.prototype.showContextualMenu = function(a, d) {
    ...
    document.body.appendChild(c); /*http://www.jrromero.net/tools/jsUML2*/
    this._diagram._divMenu = c;
    c.style.top = d + "px";
    c.style.left = a + "px"
};
...
//constructeur non paramétré -->initialisation
Diagram.prototype.initialize = function(f, e, b, d, c, a) {
    if (JSFun.isNumber(f) && e.nodeName == "DIV" && b instanceof CanvasRenderingContext2D && d instanceof CanvasRenderingContext2D && c > 0 && a > 0) {
        this._id = f; //identification diagramme
        this._div = e; // html document node--div --where is located the diagram
        this._mainContext = b; //balise canvas (html5) --getcontext
        this._motionContext = d;// context canvas drawing diagram animations
        this._width = c;
        this._height = a;
        return true
    } else {
...
//importantééé!! return an XML Object ( the tree structure diagram in XML format)
Diagram.prototype.getXML = function(c) {
    this._enumerateElements();
    this._sortNodesByArea();
    if (this._alone) {
        //DOMParser: parse XML from strings into DOM trees //string:"<" + this.getType() + "/>"
        //--expérimentatle!
...
Diagram.prototype.getXMLString = function() {
    //serialize DOM trees to strings
    return (new XMLSerializer()).serializeToString(this.getXML())
};

//XML Object--->Diagram elements contained in the Xml structure
...
//SetXMLString: string: valid xml format---> diagram element
...
//------part of the diagram validation: Setvalidelemets:Modify the type of UML elements that can be part of the diagram
...
//------part of the diagram validation: Getvalidelemets:Array Returns the type of UML elements that can be part of the diagram
...
SuperNode.prototype.deleteRegion = function(d) {
    ...
    if (this._orientation) {
        ...
        this._minHeight = this.getHeight() - e; /*http://www.jrromero.net/tools/jsUML2*/
...
//-------------------GetElementXML:  Result: node serialized in XML format ---!It is necessary to pass the parent of the element as a parameter
...
//-------- SetElementXML (XMLNode) Sets the properties of the node using the information contained in the XML node.
//---------Can be obtained of a node serialized in XML format using function getElementXML () .
